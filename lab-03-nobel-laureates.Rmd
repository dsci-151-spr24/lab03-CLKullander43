...
**There are `r nrow(nobel)` rows and `r ncol(nobel)` columns. Each row is a nobel award recipient.**
...
There are more US nobel prize winners than non-US winners. The article seems to be correct in its claim.
...
```{r}
#| label: bar-plot-with-birth-country
library(ggplot2)
...
**The data supports the article's claim. It doesn't give the exact same results, but they are very similar.**
...
```{r}
#| label: us-lauretes-born-elsewhere
nobel_living_science %>%
...
**Germany is the most common, with Japan and the UK not far behind.**
